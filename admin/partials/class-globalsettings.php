...
class GlobalSettings {
		...
		<div id="dx-eig-gallery">
			<div class="repeat">
				<div class="repeat_container">
					...
					<span class="text">*<?php esc_html_e( 'Notice : If you have now upgraded to ultimate slideshow version 1.0.1 for first time, then click remove to unset it or click the save button to preserve old slidehow and it will be saved as [myslideshow id="random"]. ', 'ultimate-slideshow' ); ?></span>
		...
		if ( isset( $get_slideshows ) && ! empty( $get_slideshows ) ) {
			foreach ( $get_slideshows as $key => $value ) {
				if ( ! is_array( $value ) && ! empty( $value ) ) {
							$key = 'random';
							$this->old_structure( $get_slideshows, $key );
							break;
	...
	/**
	 * Loads old content of version 1.0.0
	 *
	 * @param array  $arr old array of attachment ids.
	 * @param string $key slideshow name of old attachment ids.
	 */
	public function old_structure( $arr, $key ) {
						...
						<p class="no-images-message" style="display: none;"><?php esc_html_e( 'Please add images in this gallery', 'ultimate-slideshow' ); ?></p>
							<ul id='sortable1' class="gallery_images">
								...
								foreach ( $arr as $val ) {
									...
									$html  = '<li class="image attachment details ui-state-highlight" data-attachment_id="' . $val . '" data-gallery="' . $gallery_count . '">';
									$html .= '<div class="attachment-preview">
							                                <div class="thumbnail">
							                                    ' . wp_get_attachment_image( $val, 'thumbnail' ) . '
							                                </div>';
									$html .= '<a href="#" class="delete_dx_image check" title="Remove Image"><div class="media-modal-icon"></div></a></div></li>';
